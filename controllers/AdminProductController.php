<?php
/**
 * Контроллер AdminProductController
 * Управление товарами в админпанели
 */
class AdminProductController extends AdminBase
{
    /**
     * Action для страницы "Список товаров"
     */    
    public function actionIndex()
    {
        $productsList = Product::getProductsList();
        $title = 'Список товаров';

        require_once(ROOT .'/views/admin/admin_product/index.php');
        return true;
    }

    /**
     * Action для страницы "Добавить товар"
     */
    public function actionCreate()
    {
        $categoriesList = Category::getCategoriesList();
        $title = 'Добавить товар';

        if (isset($_POST['submit']))
        {
            $options['name'] = $_POST['name'];
            $options['code'] = $_POST['code'];
            $options['price'] = $_POST['price'];
            $options['category_id'] = $_POST['category_id'];
            $options['brand'] = $_POST['brand'];
            $options['availability'] = $_POST['availability'];
            $options['description'] = $_POST['description'];
            $options['is_new'] = $_POST['is_new'];
            $options['is_recommended'] = $_POST['is_recommended'];
            $options['status'] = $_POST['status'];

            $errors = self::checkCreateProduct($options);

            if (empty($errors))
            {
                $id = Product::createProduct($options);

                if ($id)
                    if (is_uploaded_file($_FILES["image"]["tmp_name"]))
                        move_uploaded_file($_FILES["image"]["tmp_name"], $_SERVER['DOCUMENT_ROOT'] . "/upload/images/products/{$id}.jpg");

                header("Location: /admin/product");
            }
        }

        require_once(ROOT.'/views/admin/admin_product/create.php');
        return true;
    }

    /**
     * Action для страницы "Редактировать товар"
     */
    public function actionUpdate($id)
    {
        $categoriesList = Category::getCategoriesList();
        $product = Product::getProductById($id);
        $title = 'Редактировать товар';

        if (isset($_POST['submit']))
        {
            $options['name'] = $_POST['name'];
            $options['code'] = $_POST['code'];
            $options['price'] = $_POST['price'];
            $options['category_id'] = $_POST['category_id'];
            $options['brand'] = $_POST['brand'];
            $options['availability'] = $_POST['availability'];
            $options['description'] = $_POST['description'];
            $options['is_new'] = $_POST['is_new'];
            $options['is_recommended'] = $_POST['is_recommended'];
            $options['status'] = $_POST['status'];

            if (Product::updateProductById($id, $options))
            {
                if (is_uploaded_file($_FILES["image"]["tmp_name"]))
                    move_uploaded_file($_FILES["image"]["tmp_name"], $_SERVER['DOCUMENT_ROOT'] . "/upload/images/products/{$id}.jpg");
                
                header("Location: /admin/product");
            }

        }

        require_once(ROOT . '/views/admin/admin_product/update.php');
        return true;
    }

    /**
     * Action для страницы "Удалить товар"
     */
    public function actionDelete($id)
    {
        $title = 'Удалить товар';

        if (isset($_POST['submit']))
        {
            Product::deleteProductById($id);
            header("Location: /admin/product");
        }

        require_once(ROOT . '/views/admin/admin_product/delete.php');
        return true;
    }

    /**
     * проверка данных с формы добавления товара
     * @return array <p>список ошибок</p> 
     */
    private static function checkCreateProduct($options)
    {
        $errors = array();

        foreach ($options as $key => $value)
            if ($value == '')
                $errors[] = "Поле $key должно быть заполненно.";

        return $errors;
    }
}